...
FILE * file_to_read;
buffer buf;
sem_t semaphore_busy;
sem_t semaphore_taken_places;
sem_t semaphore_empty_places;
...
void exit_fun(){
    ...
    //usuniecie semaforow
    if(sem_destroy(&semaphore_busy) != 0){
        perror("blad przy usuwaniu semafora");
    }
    
    if(sem_destroy(&semaphore_taken_places) != 0){
        perror("blad przy usuwaniu semafora");
    }
     
    if(sem_destroy(&semaphore_empty_places) != 0){
        perror("blad przy usuwaniu semafora");
    }
...
void * producent_fun(void * arg){
    ...
    while(1){
        //czekanie na miejsce wolne w kolejce
        if(sem_wait(&semaphore_empty_places) != 0){
            perror("blad przy uzyciu sem_wait");
            exit(1);
        }

        //czekanie na semafor zajetosci kolejki
        if(sem_wait(&semaphore_busy) != 0){
            perror("blad przy uzyciu sem_wait");
            exit(1);
        }
        ...
        if(file_end_reached == 1){            
            if(sem_post(&semaphore_taken_places) != 0){
                perror("blad przy uzyciu sem_post");
                exit(1);
            }

            if(sem_post(&semaphore_busy) != 0){
                perror("blad przy uzyciu sem_post");
                exit(1);
            }
        ...
        //zwiekszenie ilosci zejetych miejsc w kolejce
        if(sem_post(&semaphore_taken_places) != 0){
            perror("blad przy uzyciu sem_post");
            exit(1);
        }
        
        //oddanie semafora odpowiedzialnego za zejetosc kolejki
        if(sem_post(&semaphore_busy) != 0){
            perror("blad przy uzyciu sem_post");
            exit(1);
        }
...
void * consument_fun(void * arg){
    ...
    while(1){
        //czekanie az cos bedzie w kolejce
        if(sem_wait(&semaphore_taken_places) != 0){
            perror("blad przy uzyciu sem_wait");
            exit(1);
        }
        ...
        if(buf.number_of_occupied_possitions == 0 && file_end_reached == 1){
            exit(0);
        }
        ...
        //zwiekszenie ilosci pustych miejsc w kolejce
        if(sem_post(&semaphore_empty_places) != 0){
            perror("blad przy uzyciu sem_post");
            exit(1);
        }
...
void init_multithread_communication(){
    ...
    if(sem_init(&semaphore_busy, 0, 0) != 0){
        perror("blad przy inicjalizacji semafora");
        exit(1);
    }

    if(sem_init(&semaphore_taken_places, 0, 0) != 0){
        perror("blad przy inicjalizacji semafora");
        exit(1);
    }

    if(sem_init(&semaphore_empty_places, 0, buf.queue_size) != 0){
        perror("blad przy inicjalizacji semafora");
        exit(1);
    }
}
...
void create_threads(int P, int K){
    ...
    //zwiekszenie na 1 wartosci semafora busy
    if(sem_post(&semaphore_busy) != 0){
        perror("blad przy zwiekszaniu semafora");
        exit(1);
    }
}
